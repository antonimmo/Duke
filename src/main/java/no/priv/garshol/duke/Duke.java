...
public class Duke {
  ...
  static abstract class AbstractLinkFileListener extends AbstractMatchListener {
    protected Collection<Property> idprops;
	...
	// Método añadido para imprimir también la confidencia en el archivo "linkfile"
    public abstract void printMatchOnLinkFile(Record r1, Record r2, double confidence) throws IOException;
	
	public abstract void link(String id1, String id2) throws IOException;
    
	// Para volver al comportamiento anterior, se comenta la línea que manda a llamar printMatchOnLinkFile()
	// y se descomentan los for() anidados
    public void matches(Record r1, Record r2, double confidence) {
      try {
		  printMatchOnLinkFile(r1, r2, confidence);
  ...
  static class LinkFileListener extends AbstractLinkFileListener {
	...
	// añadidos para separar el archivo formateado
    private Writer formattedOut;
    private LinkFileWriter formattedWriter;
    ...
    public LinkFileListener(String linkfile, Collection<Property> idprops, boolean interactive, String testfile) throws IOException {
	  ...
	  // añadidos para separar los files y que no choquen
      this.formattedOut = new FileWriter("formatted_"+linkfile, false);
      this.formattedWriter = new LinkFileWriter(formattedOut);
    ...
    private static String record2str(Record r) {
	...
	// Implementación de método añadido
	public void printMatchOnLinkFile(Record r1, Record r2, double confidence) throws IOException{
        formattedWriter.writeFormatted(record2str(r1), record2str(r2), true, confidence);
        formattedOut.flush(); // make sure we preserve the data
	}
  ...
  static class NTriplesLinkFileListener extends AbstractLinkFileListener {
	...
	// Para cumplir con el prototipo inducido por la clase abstracta AbstractLinkFileListener
	public void printMatchOnLinkFile(Record r1, Record r2, double confidence) throws IOException{
		// No implementado
	}
  }
